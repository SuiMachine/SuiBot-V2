...
namespace CharacterAi.Client
{
	public class CharacterAiClient : IDisposable
	{
		...
		/// authToken : client
		/// </summary>
		private ConcurrentDictionary<string, WsConnection> WsConnections { get; } = new ConcurrentDictionary<string, WsConnection>();
		...
		public CharacterAiClient()
		{
			...
			(string header, string value)[] defaultHeaders = new (string header, string value)[]
			{
		...
		public async Task<AuthorizedUser> LoginByLinkAsync(string link)
		{
			...
			while (true)
			{
				...
				try
				{
					...
					content = content.Substring(content.IndexOf("https://auth", StringComparison.Ordinal));
					content = Regex.Replace(content, "\"]\\)<\\/script><script>self\\.__next_f\\.push\\(\\[\\d,\"", string.Empty);
					content = content.Substring(0, (content.IndexOf("\",", StringComparison.Ordinal) - 1));

					var parts = content.Split(new string[] { "oobCode=", "\\u0026apiKey=", "\\u0026" }, StringSplitOptions.RemoveEmptyEntries);
					oobCode = parts[1];
					apiKey = parts[2];
					email = parts[3].Split(new string[] { "email%3D", "%26" }, StringSplitOptions.RemoveEmptyEntries)[1];
